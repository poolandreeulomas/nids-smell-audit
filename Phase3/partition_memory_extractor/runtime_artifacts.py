"""Artifact persistence for Partition Memory Extractor component runs.

Persists: partition_memory.json, prompt.txt, raw_response.txt, runtime_metrics.json.
...
DEFAULT_PARTITION_MEMORY_DIR = (
    Path(__file__).resolve().parent.parent / "logs" / "partition_memory_runs"
)
_RUN_INDEX_PATTERN = re.compile(r"^partition_memory_run_(?P<index>\d{3})_")
...
def ensure_partition_memory_runs_dir(
    log_dir: str | Path | None = None,
) -> Path:
    target = Path(log_dir) if log_dir else DEFAULT_PARTITION_MEMORY_DIR
    target.mkdir(parents=True, exist_ok=True)
    return target


def get_next_partition_memory_run_index(
    log_dir: str | Path | None = None,
) -> int:
    runs_dir = ensure_partition_memory_runs_dir(log_dir)
    max_index = 0
...
def build_partition_memory_run_basename(
    batch_id: str,
    *,
    ...
    visible_index = (
        ...
        else get_next_partition_memory_run_index(log_dir)
    )
    return "partition_memory_run_{index:03d}_{day_month}_{batch_tag}".format(
        index=visible_index,
...
def build_partition_memory_artifact_paths(
    *,
    batch_id: str,
    log_dir: str | Path | None = None,
) -> dict[str, Path]:
    runs_dir = ensure_partition_memory_runs_dir(log_dir)
    run_dir = runs_dir / build_partition_memory_run_basename(
    ...
    return {
        ...
        "partition_memory_path": run_dir / "partition_memory.json",
...
def save_partition_memory_artifacts(
    *,
    ...
    write_json(artifact_paths["partition_memory_path"], partition_memory)
...
def list_partition_memory_run_dirs(
    *,
    log_dir: str | Path | None = None,
    limit: int | None = None,
) -> list[Path]:
...
def load_partition_memory_bundle(
    run_dir: str | Path,
) -> dict[str, Any]:
    path = Path(run_dir)
    ...
    return {
        ...
        "partition_memory": load_json(artifact_paths["partition_memory_path"]),
