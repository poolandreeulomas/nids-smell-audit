...
def get_df_cache_events(df: pd.DataFrame) -> list[dict[str, Any]]:
    """Return cache events recorded for deterministic tool execution."""
    events = df.attrs.get("_phase2_tool_cache_events")
    if not isinstance(events, list):
        events = []
        df.attrs["_phase2_tool_cache_events"] = events
    return events


def snapshot_cache_events(df: pd.DataFrame) -> int:
    """Return the current cache-event cursor for later diffing."""
    return len(get_df_cache_events(df))


def collect_cache_events_since(df: pd.DataFrame, cursor: int) -> list[dict[str, Any]]:
    """Return cache events recorded after the provided cursor."""
    return sanitize_json_like(get_df_cache_events(df)[cursor:])
...
def get_or_cache(df: pd.DataFrame, cache_key: str, factory: Callable[[], Any]) -> Any:
    ...
    events = get_df_cache_events(df)
    cache_status = "hit"
    if cache_key not in cache:
        cache[cache_key] = factory()
        cache_status = "miss"
    events.append({"cache_key": cache_key, "status": cache_status})
    if len(events) > 200:
        del events[:-200]
