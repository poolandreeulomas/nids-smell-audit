...
def _build_investigation_memory(
    ...
    return {
        ...
        "completion_criteria": list(memory.get("completion_criteria", [])),
...
def build_worker_prompt(
    *,
    ...
    if is_reasoning_stage:
        ...
        step_rules.extend(
            [
                ...
                "Action windows exist for evidence acquisition.",
                "The default expectation during an action window is to acquire evidence, not to preserve budget.",
                "",
                "When multiple bounded and non-redundant actions can reduce uncertainty, return all of them in the same action window.",
                "Do not stop after identifying a single useful action if additional useful actions remain available.",
                "",
                "Evaluate uncertainty globally across the current task and ask:",
                "- What remains unresolved?",
                "- What evidence would reduce that uncertainty?",
                "- Which available actions can obtain that evidence?",
                "",
                "Prefer the set of actions that maximizes uncertainty reduction rather than the first action that appears useful.",
                "",
                "ACTION BUDGET UTILIZATION",
                "Action windows exist for evidence acquisition.",
                "The default expectation is to acquire evidence.",
                "Do not preserve budget when useful evidence can still be obtained.",
                "When multiple bounded and non-redundant actions can reduce uncertainty, return all of them.",
                "Do not stop after identifying a single useful action if additional useful actions remain available.",
                "Evaluate:",
                ...
                "and select the set of actions that maximizes uncertainty reduction.",
                "",
                "MULTI-ACTION GUIDANCE",
                "Good:",
                ...
                "",
                "CONTINUE JUSTIFICATION RULE",
                "Returning decision='continue' during an action window requires justification.",
                "Only return continue when:",
                "- all important task questions are resolved,",
                "- all meaningful verification opportunities are exhausted,",
                "- remaining actions would be substantially redundant,",
                "- no meaningful uncertainty reduction remains possible.",
                "If useful evidence can still be acquired, prefer actions.",
                "",
                "CONSISTENCY RULE",
                "Do not claim:",
                "- verification opportunities remain",
                "- uncertainty remains",
                "- confirmation remains possible",
                "and then return decision='continue'.",
                "These statements require corresponding action requests unless no bounded action can address them.",
                "",
                "Previous actions are already recorded in EXECUTION_HISTORY.",
    ...
    sections.extend([
        ...
        "'completion_criteria' define what constitutes sufficient resolution.",
        "'constraints' are boundaries the investigation must respect.",
    ])
