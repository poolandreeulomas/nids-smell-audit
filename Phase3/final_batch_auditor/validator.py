...
def validate_final_batch_audit_input(
    ...
    *,
    ...
    if not isinstance(audit_context, dict):
        errors.append(_error("audit_context", "audit_context must be an object."))
    ...
    if not isinstance(final_audit_input, dict):
        errors.append(_error("final_audit_input", "final_audit_input must be an object."))
        final_audit_input = {}
    else:
        unsupported_fields = sorted(set(final_audit_input.keys()) - _FINAL_AUDIT_INPUT_FIELDS)
        if unsupported_fields:
            errors.append(
                _error(
                    "final_audit_input",
    ...
    if not _is_non_empty_string(batch_id):
        errors.append(_error("final_audit_input.batch_id", "batch_id must be a non-empty string."))
    ...
    if not _is_non_empty_string(final_state_ref):
        errors.append(
            _error("final_audit_input.final_state_ref", "final_state_ref must be a non-empty string.")
        )
    ...
    for index, item in enumerate(round_artifact_refs):
        ...
        if not isinstance(item, dict):
            errors.append(_error(field_prefix, "Each round_artifact_ref must be an object."))
            continue
        unsupported_fields = sorted(set(item.keys()) - _ROUND_ARTIFACT_REF_FIELDS)
        if unsupported_fields:
        ...
        if not _is_non_empty_string(component_name):
            errors.append(_error(f"{field_prefix}.component_name", "component_name must be a non-empty string."))
    ...
    for index, item in enumerate(hypothesis_history_refs):
        ...
        if not isinstance(item, dict):
            errors.append(_error(field_prefix, "Each hypothesis_history_ref must be an object."))
            continue
    ...
    if not isinstance(final_state_summary, dict):
        errors.append(_error("final_state_summary", "final_state_summary must be an object."))
        final_state_summary = {}
    else:
        unsupported_fields = sorted(set(final_state_summary.keys()) - _FINAL_STATE_SUMMARY_FIELDS)
        if unsupported_fields:
            errors.append(
                _error(
                    "final_state_summary",
    ...
    if not _is_non_empty_string(summary_batch_id):
        errors.append(_error("final_state_summary.batch_id", "batch_id must be a non-empty string."))
    ...
    for key in (
        ...
        if value is None or value < 0:
            errors.append(_error(f"final_state_summary.{key}", f"{key} must be an integer greater than or equal to 0."))
    ...
    for index, item in enumerate(hypothesis_snapshots):
        ...
        if not isinstance(item, dict):
            errors.append(_error(field_prefix, "Each hypothesis snapshot must be an object."))
            continue
        unsupported_fields = sorted(set(item.keys()) - _HYPOTHESIS_SNAPSHOT_FIELDS)
        if unsupported_fields:
            errors.append(_error(field_prefix, f"Unsupported fields: {unsupported_fields}."))
        for key in ("hypothesis_id", "status", "summary", "last_updated_round"):
            if key == "last_updated_round":
                if item.get(key) is not None and not isinstance(item.get(key), str):
                    errors.append(_error(f"{field_prefix}.{key}", f"{key} must be a string."))
        ...
        if revision_count is None or revision_count < 0:
            errors.append(_error(f"{field_prefix}.revision_count", "revision_count must be an integer greater than or equal to 0."))
    ...
    if not isinstance(round_history_summary, list) or not round_history_summary:
        errors.append(_error("round_history_summary", "round_history_summary must be a non-empty list."))
        round_history_summary = []
    for index, item in enumerate(round_history_summary):
        field_prefix = f"round_history_summary[{index}]"
        if not isinstance(item, dict):
            errors.append(_error(field_prefix, "Each round history summary must be an object."))
            continue
        ...
        for key in (
            ...
            if value is None or value < 0:
                errors.append(_error(f"{field_prefix}.{key}", f"{key} must be an integer greater than or equal to 0."))
        refs = _string_list(item.get("traceability_refs"), allow_empty=False)
        if refs is None:
            errors.append(_error(f"{field_prefix}.traceability_refs", "traceability_refs must be a non-empty list of strings."))
    ...
    if not _is_non_empty_string(process_batch_id):
        errors.append(_error("process_signal_summary.batch_id", "batch_id must be a non-empty string."))
    ...
    for key in ("state_version", "round_count", "critic_run_count"):
        ...
        if value is None or value < 0:
            errors.append(_error(f"process_signal_summary.{key}", f"{key} must be an integer greater than or equal to 0."))
    if not isinstance(process_signal_summary.get("terminal_gate_expected"), bool):
        errors.append(_error("process_signal_summary.terminal_gate_expected", "terminal_gate_expected must be a boolean."))
...
def validate_debugging_audit_report(
    ...
    *,
    ...
    raw = debugging_audit_report if isinstance(debugging_audit_report, dict) else {}
    errors: list[dict[str, str]] = []
    warnings: list[dict[str, str]] = []
    forbidden_language_hits: list[dict[str, str]] = []

    if not isinstance(debugging_audit_report, dict):
        errors.append(_error("debugging_audit_report", "debugging_audit_report must be an object."))

    unsupported_fields = sorted(set(raw.keys()) - _DEBUGGING_AUDIT_REPORT_FIELDS)
    if unsupported_fields:
        errors.append(
            _error(
    ...
    if not _is_non_empty_string(batch_id):
        errors.append(_error("debugging_audit_report.batch_id", "batch_id must be a non-empty string."))
    elif str(batch_id).strip() != expected_batch_id:
        errors.append(
            _error(
    ...
    for key in ("trajectory_summary", "hypothesis_summary", "failure_summary"):
        ...
        if not _is_non_empty_string(value):
            errors.append(_error(f"debugging_audit_report.{key}", f"{key} must be a non-empty string."))
            continue
        ...
        forbidden_language_hits.extend(_collect_forbidden_language(f"debugging_audit_report.{key}", value))

    for list_field in ("surviving_contradictions", "open_pressures"):
        values = _string_list(raw.get(list_field))
        if values is None:
            errors.append(_error(f"debugging_audit_report.{list_field}", f"{list_field} must be a list of strings."))
            continue
    ...
    traceability_refs = _string_list(raw.get("traceability_refs"), allow_empty=False)
    if traceability_refs is None:
        ...
        unknown_refs = sorted(set(traceability_refs) - set(known_traceability_refs))
        if unknown_refs:
            errors.append(
                _error(
                    "debugging_audit_report.traceability_refs",
