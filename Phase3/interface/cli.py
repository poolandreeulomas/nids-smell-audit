...
class NidsAgentCli:
    ...
    def _main_menu(self) -> str | None:
        ...
        choice = self._read_letter_choice({"R", "P", "J", "L", "V", "E", "M", "Q"})
        routes = {
            "P": "phase3a",
            "J": "judge",
            "L": "latest",
    ...
    def _phase3a_components_menu(self) -> str:
        ...
        choice = self._read_menu_choice({"11", "B", "Q"})
        if choice == "Q":
            self._quit()
            return "phase3a"
        if choice == "B":
    ...
    def _tools_menu(self) -> str:
        ...
        if self._last_tool_run is not None:
            latest_tool_run_name = Path(self._last_tool_run.artifact_paths.get("component_run_path", "")).parent.name or None
        ...
        choice = self._read_letter_choice({"R", "I", "L", "V", "B", "Q"})
        if choice == "Q":
            self._quit()
            return "tools"
        if choice == "B":
    ...
    def _run_tool_flow(self) -> str:
        ...
        target_scope, input_refs = self._prompt_tool_scope_and_inputs(tool_name)
        ...
        while True:
            ...
            choice = self._read_menu_choice({"1", "2", "3", "4", "5", "B", "Q"})
            if choice == "Q":
                self._quit()
                return "tools"
            if choice == "B":
    ...
    def _prompt_tool_scope_and_inputs(self, tool_name: str) -> tuple[str | None, dict[str, Any]]:
        ...
        if not supported_scopes:
            self._show_error(f"tool '{tool_name}' has no supported scopes configured")
            return None, {}
        ...
        if len(supported_scopes) == 1:
            ...
            target_scope = self._read_non_empty_input(
                "Enter target scope ({}) or B to go back.".format(", ".join(supported_scopes))
            )
            if target_scope.upper() == "B":
                return None, {}
            if target_scope not in supported_scopes:
                self._show_error(f"target scope must be one of: {', '.join(supported_scopes)}")
                return None, {}

        input_refs: dict[str, Any] = {}
        if tool_name != "duplication_analysis":
            feature_name = self._read_non_empty_input("Enter feature name, or B to go back.")
            if feature_name.upper() == "B":
                return None, {}
            input_refs["feature_name"] = feature_name

        if tool_name == "feature_relation" and target_scope == "feature_pair":
            related_feature_name = self._read_non_empty_input("Enter related feature name, or B to go back.")
    ...
    def _build_tool_run_context(self, bundle: dict[str, Any]) -> ToolRunContext:
        ...
        if not component_run and "artifact_paths" in bundle and bundle["artifact_paths"].get("component_run_path"):
            component_run = load_json(bundle["artifact_paths"]["component_run_path"])
        ...
        return ToolRunContext(
            ...
            tool_capability_record=dict(bundle.get("tool_capability_record", {})),
            normalized_inputs=dict(bundle.get("normalized_inputs", {})),
            raw_tool_output=dict(bundle.get("raw_tool_output", {})),
            parsed_output=dict(bundle.get("tool_result", bundle.get("parsed_output", {}))),
            validation_report=dict(bundle.get("validation_report", {})),
            tool_metrics=dict(bundle.get("tool_metrics", {})),
            cache_record=dict(bundle.get("cache_record", {})) if bundle.get("cache_record") else None,
        )

    def _render_tool_run_review(self, run_context: ToolRunContext) -> None:
        run_name = Path(run_context.artifact_paths.get("component_run_path", "")).parent.name or "tool_run"
        summary_pairs = [
    ...
    def _handle_tool_review_choice(self, choice: str, run_context: ToolRunContext) -> None:
        ...
        metrics = [
            ...
            ("cache_event_count", str(run_context.tool_metrics.get("cache_event_count", 0))),
            ("fresh_execution", str(run_context.tool_metrics.get("fresh_execution", True))),
            ("schema_version", str(run_context.tool_metrics.get("schema_version", "unknown"))),
        ]
        self._render(
            render_technical_details(
                ...
                metrics=metrics,
                tools_used=[str(run_context.component_run.get("tool_name", "unknown"))],
                artifact_paths={
    ...
    def _view_tool_runs_menu(self) -> str:
        while True:
            ...
            self._render(render_recent_tool_runs(recent_runs, self._view_tool_runs_limit))
            if not recent_runs:
                choice = self._read_letter_choice({"B", "Q"})
                if choice == "Q":
                    self._quit()
            ...
            valid_choices = {str(index) for index in range(1, len(recent_runs) + 1)} | {"N", "B", "Q"}
            ...
            while True:
                self._render_tool_run_review(run_context)
                review_choice = self._read_menu_choice({"1", "2", "3", "4", "5", "B", "Q"})
                if review_choice == "Q":
                    self._quit()
                    return "tools"
                if review_choice == "B":
